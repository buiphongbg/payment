<?php
return [
    'driver' => env('PAYMENT_DRIVER', 'napas'),
    'napas' => [
        'merchant_id' => env('NAPAS_MERCHANT_ID'),
        'secure_hash' => env('NAPAS_SECURE_HASH'),
        'access_code' => env('NAPAS_ACCESS_CODE'),
        'env'         => env('NAPAS_ENV', 'production')
    ],

    'onepay' => [
        'merchant_id' => env('ONEPAY_MERCHANT_ID', 'ONEPAY'),
        'secure_secret' => env('ONEPAY_SECURE_SECRET', 'A3EFDFABA8653DF2342E8DAC29B51AF0'),
        'access_code' => env('ONEPAY_ACCESS_CODE', 'D67342C2'),
        'env'         => env('ONEPAY_ENV', 'sandbox')
    ],

    'payoo' => [
        'key' => env('PAYOO_KEY'),
        'secret' => env('PAYOO_SECRET'),
        'env'    => env('PAYOO_ENV', 'production')
    ],

    'baokim' => [
        'email_business'  => env('BAOKIM_EMAIL_BUSINESS', 'dev.baokim@bk.vn'),
        'merchant_id'     => env('BAOKIM_MERCHANT_ID', '647'),
        'secure_pass'     => env('BAOKIM_SECURE_PASS', 'ae543c080ad91c23'),
        'api_user'        => env('BAOKIM_API_USER', 'merchant'),
        'api_pwd'         => env('BAOKIM_API_PWD', '1234'),
        'env'             => env('BAOKIM_ENV', 'sandbox'),
        'private_key'     => env('BAOKIM_PRIVATE_KEY', '-----BEGIN PRIVATE KEY-----

TU2Q8idAb77K7mvVguA3LDhN
-----END PRIVATE KEY-----')
    ],
];